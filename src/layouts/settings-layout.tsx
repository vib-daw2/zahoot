        ...
        <div className='w-full flex pt-12 px-4 h-full flex-row'>
            <div className=' w-full max-w-md text-white h-full min-h-[calc(100vh-6rem)] border-r border-r-slate-700 flex flex-col justify-start items-start mt-4'>
                {options.map((option, i) => (
                    <NavLink to={option.href} className={({ isActive }) => `py-3 px-4 w-full cursor-pointer hover:bg-slate-800 hover:border-l-4 hover:border-cyan-400 ${isActive ? "bg-slate-800 border-l-2 border-cyan-400" : ""}`}>{option.title}</NavLink>
                ))}
            </div>
